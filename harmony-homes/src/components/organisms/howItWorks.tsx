...
export default function HowItWorks() {
  return (
    <div className="bg-[#F4F4F5] text-black flex flex-col justify-center items-center font-sans py-12 space-y-5">
      <div className="text-center space-y-1">
        <h2 className="text-3xl font-bold">How Harmony Works</h2>
        <p className="text-[#71717A] max-w-3xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center space-y-5 md:space-y-0 md:space-x-10 px-5 lg:px-16 mt-5 lg:max-w-6xl mx-auto">
        {howItWorks.map((data) => (
          <div key={data.id} className="space-y-1">
            <span className="inline-block bg-[#18181B] text-white rounded-md p-2">
              {data.icon}
            </span>
            <h3 className="font-bold text-xl">{data.title}</h3>
            <p className="text-[#71717A]">{data.description}</p>
          </div>
        ))}
      </div>
    </div>
